...
contract QubeBridge is Ownable, ReentrancyGuard, Pausable {
    ...
    // --- State Variables ---
    uint256 public immutable sourceChainId;  // Chain ID where this contract is deployed
    address public controller;
    address public multisig;
    address public feeRecipient;  // New: Address to receive bridge fees
    uint256 public feePercent;
    uint256 public unpauseDelay = 48 hours;
    uint256 private _unpauseTime;
    bool private _paused;  // <-- New: Pause state
    ...
    event FeeRecipientUpdated(address newRecipient);  // New: Track fee recipient changes
    ...
    constructor(
        ...
        address _controller,
        address _multisig,
        address _feeRecipient  // New: Optional (defaults to controller if zero)
    ) Ownable(_initialOwner) {
        require(_controller != address(0), "Bridge: invalid controller");
        require(_multisig != address(0), "Bridge: invalid multisig");
        require(_sourceChainId != 0, "Bridge: invalid chain ID");
        sourceChainId = _sourceChainId;
        controller = _controller;
        multisig = _multisig;
        feeRecipient = _feeRecipient == address(0) ? _controller : _feeRecipient;  // Default to controller
        feePercent = 0;

        // Automatically support the source chain
        _addSupportedChain(_sourceChainId);
    }
    ...
    function bridge(
        ...
        require(destinationChainId != sourceChainId, "Bridge: same chain");
        require(isSupportedToken(tokenAddress), "Bridge: token not supported");
        require(isSupportedChain(destinationChainId), "Bridge: chain not supported");
        ...
        uint256 nonce = ++nonces[sourceChainId][destinationChainId];
    ...
    */
    function setMintableToken(address token, bool isMintable) external {
    ...
    function addSupportedChain(uint256 chainId) external {
        ...
        require(chainId != sourceChainId, "Bridge: cannot add source chain");
        _addSupportedChain(chainId);
    }

    function removeSupportedChain(uint256 chainId) external {
        require(msg.sender == controller, "Bridge: unauthorized");
        require(chainId != sourceChainId, "Bridge: cannot remove source chain");
